...
#' set2 <- set; set2$info <- set2$info %>% filter(!str_detect(namespace,'L1000'))
#' 
#' eset <- oSEA(data, set, ig)
#' eset$ig
#' eset %>% oSEAextract() %>% write_delim('results_eset.txt','\t')
#' eset %>% oSEAextract() %>% filter(distance=='DiseaseDrug') %>% oSEAforest(top=2)
#' eset %>% oSEAextract() %>% filter(adjp<0.01) %>% group_by(namespace) %>% top_n(2,-adjp) %>% ungroup() %>% oSEAwheel(ig, fixed=F, edge.color='namespace', leaf.label.orientation='outwards', leaf.label.expansion=1.15,limit.expansion=2, leaf.label.size=1.5)
#'
#' # ggraph 
...
oSEA <- function(data, set, ig=NULL, background=NULL, size.range=c(10,2000), min.overlap=5, which.distance=NULL, test=c("fisher","hypergeo","binomial"), background.annotatable.only=NULL, p.tail=c("one-tail","two-tails"), p.adjust.method=c("BH", "BY", "bonferroni", "holm", "hochberg", "hommel"), ontology.algorithm=c("none","pc","elim","lea"), elim.pvalue=1e-2, lea.depth=2, path.mode=c("all_paths","shortest_paths","all_shortest_paths"), true.path.rule=FALSE, verbose=TRUE)
{
  ...
  ####################################################################################
    ...
    if(is(set,"SET")){
    	name <- member <- NULL
        anno <- set$info %>% dplyr::mutate(member=purrr::map(member,~sort(.x))) %>% dplyr::select(name,member) %>% tibble::deframe()
    ...
    if(is(subg,"igraph")){
    	...
    	set_info <- subg %>% oIG2TB("nodes") %>% dplyr::transmute(name=name, id=id, distance=distance, namespace=namespace)
    }else{
    	if(is(set,"SET")){
			id <- name <- distance <- namespace <- NULL
			set_info <- set$info %>% dplyr::semi_join(tibble::tibble(name=terms), by="name") %>% dplyr::select(name, id, distance, namespace)
    ...
    info <- dplyr::bind_cols(set_info, tibble::tibble(fc=fcs, zscore=zscores, pvalue=pvals, adjp=adjpvals, or=ors, CIl=CIl, CIu=CIu, overlap=overlaps, anno=gs)) %>% dplyr::mutate(nA=purrr::map_int(anno,~length(.x)), nO=purrr::map_int(overlap,~length(.x))) %>% dplyr::select(name,id,nA,nO,fc,zscore,pvalue,adjp,or,CIl,CIu,distance,namespace,overlap) %>% dplyr::arrange(pvalue,-or)
